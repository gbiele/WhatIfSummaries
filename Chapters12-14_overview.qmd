...
3. G-estimation^[Adjusts only for confounding, but can be combined with IP weighting to adjust for selection bias]:  Estimate the causal effect in a treatment model by using the changeability assumption and finding the effect value under which treatment is independent of the effect value. 
...
Because exchangeability does not hold in the study sample, we create a pseudopopulation in which conditional exchangeability holds.

> Informally, the pseudo-population is created by weighting each individual by the inverse of the conditional probability of receiving the treatment level _that she indeed received_. The individual-specific IP weights for treatment $A$ are defined as $W^A = 1/f (A|L)$

$f (A|L)$ can be a simple regression model, often called the _treatment model_.
...
> inverse of the conditional [estimated] probability of receiving the treatment level _that she indeed received_. 
...
nhefs %>% 
  .[, p_treat := 
      predict(treatment_model,
              type = "response")] %>% 
  .[qsmk == 1, w := 1/p_treat] %>%  # treated
  .[qsmk == 0, w := 1/(1-p_treat)]  # untreated
```
:::
...
Non-stabilized weights can amplify the effect of extreme data points and result in estimates with larger standard error.

> weights are $\textrm{Pr}[A = 1] /f (A|L)$ for the treated and $\textrm{Pr}[A = 0] /f (A|L)$ for the untreated or

$$
sw = \frac{\textrm{Pr}[A=a]}{\textrm{Pr}[A|L]}
$$
...
## IP weighting: Calculating stabelised weights
...
__IPW "balances" treated and untreated only wrt. observed covariates!__

## IP weighting: Effect estimation

Simply use IP weights in a regression model, no covariates needed!
...
We estimate counterfactual outcomes for everyone.
...
strat.outcome = 
  ...
  .[, strat.wght := N/sum(N)] %>%            # weights for STRATA
  na.omit()                                  
...
```{r}
#| echo: true
fmean(strat.outcome$m.qsmk1,w = strat.outcome$strat.wght) -
fmean(strat.outcome$m.qsmk0,w = strat.outcome$strat.wght)
```
...
> To obtain parametric estimates of $E[Y |A = a, L = l]$ in each of the
millions of strata defined by $L$, we fit a linear regression model for the mean
weight gain with treatment $A$ and ... confounders $L$ included as covariates.
...
Generate data for predictions under both treatment conditions.
```{r}
#| echo: true
newdataA0 = nhefs %>% 
  .[, .(sex, race, age, education, smokeintensity, smokeyrs, exercise, active, wt71)]
newdataA0 = copy(newdataA0)[, qsmk := 0]
newdataA1 = copy(newdataA0)[, qsmk := 1]
```

```{r}
head(newdataA0[, .(sex, race, age, education, smokeyrs, exercise, active, wt71, qsmk)])
head(newdataA1[, .(sex, race, age, education, smokeyrs, exercise, active, wt71, qsmk)])
```
...
```{r}
#| echo: true
pred_qsmk0 = predict(outcome_model, newdataA0)
pred_qsmk1 = predict(outcome_model, newdataA1)
```
...
- We average over all individuals in the data set, without weights.
...
## IPW or standardization?

- Are only the same when they are estimated without model (or with a non-parametric model)
- Model-based estimates can differ, in contrasts to non-parametric estimates, because the models estimate different things:
  - _Treatment model_ for IPW: $\textrm{Pr}[A=a|L]$^[Does not need outcome model, because it is simply calculated as the differences of weighted means of observed outcomes!]
  - _Outcome model_ for standardisation: $E[Y|A=a,L=l]$
- When IPW and standardization lead to different results, this points to misspecification of either of the models
  - parametric form, likelihood, included variables ...
...
structural model is then the (unweighted) outcome regression model that 
...
print(AIPW_SL$result[1:3,], digits = 2) # show results
```
...
Definition: Treatment value is independent of potential outcomes, it must be independent of what the outcome had been without the treatment.

$$
\textrm{Pr}[A=1|Y^{a=0},L] = \textrm{Pr}[A=1|L]
$$
...
$$^[This is just a different way to decribe a logistic regression]
...
curve(dnorm(x),-3,5, ylab = "density", xlab = "weight change", xaxt = "n", yaxt = "n", bty = "n")
axis(1, at = c(-10,10))
axis(2, at = c(-10,10))
...
2. To learn the value of $\psi_1$ we exploit that under exchangeability the treatment should be independent of the potential outcomes, it should be independent of what the outcome had been without the treatment.

$$
\textrm{logit Pr}  [A=1|Y^{a=0},L] = \alpha_0 + \color{red}{\alpha_1}\color{blue}{Y^{a=0}} + \alpha_2L
$$

That is, we need to find a value for $\color{blue}{Y^{a=0}}$ such that when we estimate the model $\color{red}{\alpha_1}$ becomes $0$. We do this buy trying out (in a smart manner) different values for $\color{blue}{Y^{a=0}}$ for those that actually received treatment.^[There are fancy ways to do this, but here the point is that we understand the idea of why we can estimate a type of treatment model to get the value of the treatment effect.]